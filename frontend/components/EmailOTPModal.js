...
export default function EmailOTPModal({
  ...
  const scaleAnim = useRef(new Animated.Value(0.8)).current;
  const fadeAnim = useRef(new Animated.Value(0)).current;
  const isVerifyingRef = useRef(false); // Add ref to prevent double verification

  // Watch for verification success/error from parent
  useEffect(() => {
    if (verificationSuccess) {
      console.log('✅ Verification successful, showing success animation');
      setShowSuccess(true);
  ...
  const handleOTPChange = (value) => {
    ...
    // FIXED: Use cleanValue directly instead of state
    if (cleanValue.length === 6 && !isLoading) {
  console.log('🔄 Auto-verifying with OTP:', cleanValue);

  // reset stale verification state
 // isVerifyingRef.current = false;
  ...
  const handleVerify = (otpValue) => {
    console.log('🔐 handleVerify called with:', otpValue);
    console.log('Current isLoading:', isLoading);
    console.log('Current isVerifyingRef:', isVerifyingRef.current);
    
    if (!otpValue || otpValue.length !== 6) {
      setError('Please enter all 6 digits');
      console.log('❌ Invalid OTP length:', otpValue?.length);
      return;
    }
    ...
    console.log('📤 Calling onVerify with OTP:', otpValue);
  ...
  const handleResend = () => {
    if (isLoading) {
      console.log('⏳ Cannot resend while verifying');
      return;
    }
    
    if (resendCount >= 2) {
      console.log('⚠️ Max resend attempts reached');
  ...
  const handleSuccessClose = () => {
    console.log('✅ Success animation complete, closing modal');
    setShowSuccess(false);
    Keyboard.dismiss();
    // isVerifyingRef.current = false;
    onClose();
          ...
          >
            ...
            <View style={styles.resendSection}>
              ...
              {canResend && resendCount < 2 ? (
                ...
                <Text style={styles.resendLimitText}>Max resend attempts reached</Text>
              ) : (
                <Text style={styles.resendDisabledText}>Resend in {formatTime(timeLeft)}</Text>
              )}
            </View>
          </Animated.View>
        </View>
...
const styles = StyleSheet.create({
  ...
  headerIconSection: { alignItems: 'center', marginBottom: 24 },
  largeIconContainer: {
    width: 100,
    height: 100,
    borderRadius: 50,
  ...
  title: { fontSize: 28, fontWeight: '700', color: Colors.primary, textAlign: 'center', marginBottom: 16 },
  emailSection: { marginBottom: 24 },
  instructionText: { fontSize: 14, color: '#6B7280', textAlign: 'center', marginBottom: 8, lineHeight: 20 },
  emailText: { fontSize: 15, fontWeight: '700', color: Colors.primary, textAlign: 'center' },
  otpDisplayContainer: { flexDirection: 'row', justifyContent: 'space-between', gap: 8, marginBottom: 20 },
  otpBox: { flex: 1, height: 60, borderWidth: 2, borderRadius: 16, justifyContent: 'center', alignItems: 'center' },
  otpBoxText: { fontSize: 26, fontWeight: '700', color: Colors.primary },
  textInput: {
  ...
  timerBarContainer: { marginBottom: 24 },
  timerBarBackground: { height: 6, backgroundColor: '#E5E7EB', borderRadius: 3, overflow: 'hidden', marginBottom: 8 },
  timerBarFill: { height: '100%', backgroundColor: Colors.primary, borderRadius: 3 },
  timerText: { fontSize: 13, color: '#6B7280', textAlign: 'center', fontWeight: '500' },
  verifyButton: { borderRadius: 12, overflow: 'hidden', marginBottom: 20, shadowColor: Colors.primary, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.2, shadowRadius: 8, elevation: 4 },
  verifyButtonDisabled: { opacity: 0.6 },
  gradientButton: { paddingVertical: 13, flexDirection: 'row', alignItems: 'center', justifyContent: 'center' },
  verifyButtonText: { fontSize: 15, fontWeight: '700', color: Colors.white },
  resendSection: { alignItems: 'center' },
  resendQuestion: { fontSize: 14, color: '#6B7280', marginBottom: 8 },
  resendButton: { flexDirection: 'row', alignItems: 'center', backgroundColor: Colors.primary, paddingHorizontal: 16, paddingVertical: 8, borderRadius: 20, gap: 6 },
  resendButtonText: { fontSize: 14, color: Colors.white, fontWeight: '600' },
  resendDisabledText: { fontSize: 13, color: '#9CA3AF', fontStyle: 'italic' },
  resendLimitText: { fontSize: 13, color: '#EF4444', fontWeight: '500' },
});