...
const ChatListScreen = ({ navigation }) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        const transformedChats = result.conversations.map(conv => ({
          ...
          lastMessage: conv.last_message || 'No messages yet',
          timestamp: formatTimestamp(conv.last_message_time),
          originalTimestamp: conv.last_message_time, // Store for sorting
          unreadCount: conv.unread_count || 0,
          isOnline: false,
          avatar: conv.other_user.avatar,
          isPinned: false,
        ...
        // Sort chats by originalTimestamp (newest first) before setting state
        ...
        console.log('Sorted chats by latest message:', sortedChats.map(c => ({ 
          name: c.name, 
          time: c.originalTimestamp 
        })));

        setChats(sortedChats);
      } else {
        console.log('Failed to load conversations:', result);
      }
    } catch (error) {
      console.error('Load conversations error:', error);
    } finally {
  ...
  // Function to update chat order when a new message is sent
  const updateChatOrder = useCallback((conversationId, newMessage, newTimestamp) => {
    setChats(prevChats => {
      // Find the chat that was updated
      const updatedChatIndex = prevChats.findIndex(chat => chat.conversationId === conversationId);
      
      if (updatedChatIndex === -1) return prevChats;
      
      const updatedChat = {
        ...prevChats[updatedChatIndex],
        lastMessage: newMessage || prevChats[updatedChatIndex].lastMessage,
        originalTimestamp: newTimestamp || new Date().toISOString(),
        timestamp: formatTimestamp(newTimestamp || new Date().toISOString()),
      };
  ...
  const handleChatPress = (chat) => {
    ...
    navigation.navigate('ChatScreen', {
      ...
      conversationId: chat.conversationId,
      receiverPhone: chat.otherUserPhone,
      onUpdateChatOrder: updateChatOrder, // Pass callback to update order instantly
    });
  };
