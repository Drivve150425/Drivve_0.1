...
//   },
// });
// Step2.js - Updated version with proper navigation back
...
export default function Step2({
  ...
  lockedFields = [],
  loadingRoute = false,
  // Add these props to go back
  goBackToStep1 = null,  // Callback function to go back to step 1
}) {
  const mapRef = useRef(null);
  ...
  // Check if we're currently loading or have no routes
  const isLoading = loadingRoute;
  const hasNoRoutes = !routeOptions || routeOptions.length === 0;

  // Auto-select the first route if none selected and routes available
  useEffect(() => {
    if (routeOptions && routeOptions.length > 0 && !selectedRoute) {
  ...
  }, [routeOptions, selectedRoute, selectedRouteIndex, setSelectedRouteIndex]);
  ...
  // Handle going back to step 1 to edit locations
  const handleGoBackToEditLocations = () => {
    if (goBackToStep1) {
      goBackToStep1();
    }
  };
  ...
  // No routes found - Show proper message with edit button
  if (hasNoRoutes) {
    return (
      <View style={styles.container}>
        <Text style={styles.title}>Select Route</Text>
        <View style={styles.noRouteContainer}>
          ...
          <Text style={styles.noRouteSubtext2}>
            Please check your pickup and destination locations and try again.
          </Text>
        </View>
        
        <TouchableOpacity
          style={[styles.actionButton, { backgroundColor: Colors.primary, marginTop: 20 }]}
          onPress={handleGoBackToEditLocations}
          activeOpacity={0.8}
        >
