...
export default function AddNewVehicleScreen({ navigation, route }) {
  ...
  const validateRegistrationNumber = (reg: string) => {
    ...
    const cleanedReg = reg.toUpperCase().replace(/\s/g, "");
    
    // Standard format: MH12AB1234
    const standardPattern = /^[A-Z]{2}[0-9]{1,2}[A-Z]{1,2}[0-9]{4}$/;
    
    // BH Series format: YY BH #### XX
    // Example: 26BH1234AB or 26 BH 1234 AB
    const bhPattern = /^[0-9]{2}BH[0-9]{4}[A-Z]{2}$/;
    
    return standardPattern.test(cleanedReg) || bhPattern.test(cleanedReg);
  };

  const formatRegistrationNumber = (text: string) => {
    let cleaned = text.toUpperCase().replace(/[^A-Z0-9]/g, "");
    
    // Check if it might be BH series (starts with 2 digits)
    if (cleaned.length >= 2 && cleaned.substring(0, 2).match(/[0-9]{2}/)) {
      // BH Series formatting: YY BH #### XX
      if (cleaned.length >= 2 && cleaned.length <= 4) {
        cleaned = cleaned.slice(0, 2) + " " + cleaned.slice(2);
      } else if (cleaned.length > 4 && cleaned.length <= 8) {
        // After first 2 digits, add BH
        let firstTwo = cleaned.slice(0, 2);
        let remaining = cleaned.slice(2);
        if (remaining.startsWith("BH")) {
          cleaned = firstTwo + " BH " + remaining.slice(2);
        } else if (remaining.length > 0) {
          cleaned = firstTwo + " BH " + remaining;
        }
        ...
        let remaining = cleaned.slice(2);
        if (remaining.startsWith("BH")) {
          let afterBH = remaining.slice(2);
          if (afterBH.length > 4) {
            let numbers = afterBH.slice(0, 4);
            let letters = afterBH.slice(4, 6);
            cleaned = `${firstTwo} BH ${numbers} ${letters}`;
          } else {
            cleaned = `${firstTwo} BH ${afterBH}`;
          }
        } else {
          cleaned = `${firstTwo} BH ${remaining}`;
        }
    ...
    // Standard format formatting: MH12AB1234
    if (cleaned.length > 2 && cleaned.length <= 4) {
      cleaned = cleaned.slice(0, 2) + " " + cleaned.slice(2);
    } else if (cleaned.length > 4 && cleaned.length <= 6) {
      cleaned = cleaned.slice(0, 2) + " " + cleaned.slice(2, 4) + " " + cleaned.slice(4);
  ...
  const getRegistrationHint = () => {
    const cleaned = registration.replace(/\s/g, "");
    if (cleaned.length >= 2 && cleaned.substring(0, 2).match(/[0-9]{2}/)) {
      return "BH Series format: YY BH #### XX (e.g., 26 BH 1234 AB)";
    }
    return "Standard format: MH12AB1234 or BH Series: 26 BH 1234 AB";
  };

  const getRegistrationExample = () => {
    const cleaned = registration.replace(/\s/g, "");
    if (cleaned.length >= 2 && cleaned.substring(0, 2).match(/[0-9]{2}/)) {
      return "26 BH 1234 AB";
    }
    return "MH12AB1234";
  };

  const getRegistrationMaxLength = () => {
    const cleaned = registration.replace(/\s/g, "");
    if (cleaned.length >= 2 && cleaned.substring(0, 2).match(/[0-9]{2}/)) {
      return 16; // "26 BH 1234 AB" has spaces
    }
    return 13; // "MH12AB1234" with spaces
  };
  ...
  const handleSave = async () => {
    ...
    if (!validateRegistrationNumber(registration.replace(/\s/g, ""))) {
      showCustomAlert(
        "Error", 
        "Please enter a valid Registration Number\n\nStandard format: MH12AB1234\nBH Series format: 26 BH 1234 AB", 
        "error"
      );
      return;
    }
        ...
        >
          ...
          <View style={styles.inputBox}>
            ...
            <TextInput
              ...
              value={registration}
              onChangeText={handleRegistrationChange}
              placeholder={getRegistrationExample()}
              placeholderTextColor="#9CA3AF"
              autoCapitalize="characters"
              maxLength={getRegistrationMaxLength()}
            />
            <Text style={styles.hintText}>{getRegistrationHint()}</Text>
            {registration.length > 0 && !validateRegistrationNumber(registration.replace(/\s/g, "")) && (
              <Text style={styles.errorText}>
                Invalid format. Use standard (MH12AB1234) or BH Series (26 BH 1234 AB)
              </Text>
            )}
          </View>
