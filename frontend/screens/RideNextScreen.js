...
export default function RideNextScreen({ navigation, route }) {
      ...
      const ridesWithDetails = await Promise.all(
        rides.map(async (ride) => {
          ...
          return { 
            ...
            seatsAvailable: ride.seatsAvailable || 0,
            requestedSeats: requestedSeats,
            isFull: ride.isFull || (ride.seatsAvailable === 0),
          };
        })
      );
  ...
  // Calculate counts for the summary bar
  const fullRideCount = processedRides.filter(ride => (ride.seatsAvailable || 0) === 0).length;
  const availableRideCount = processedRides.filter(ride => (ride.seatsAvailable || 0) > 0).length;

  const handleCardPress = (ride) => {
    // Allow navigation even for full rides (they can't book but can see details)
    navigation.navigate('RideDetailScreen', { 
      ride,
      searchData: searchData || null,
    });
  ...
  const renderRideCard = ({ item }) => {
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() => handleCardPress(item)}
        activeOpacity={0.9}
        disabled={isSeatsInsufficient || isFull}  // ← REMOVE THIS LINE
      ...
      >
          ...
          <View style={styles.profileRow}>
            ...
            <View style={styles.profileContent}>
              <View style={styles.nameRow}>
                ...
                {/* Full Badge - shows when ride is full */}
                {isFull && (
                  <View style={styles.fullBadge}>
                    <Ionicons name="close-circle" size={12} color="#EF4444" />
                    <Text style={styles.fullBadgeText}>Full</Text>
        ...
        {/* Insufficient Seats Warning Message */}
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {/* Result Summary - Shows counts of available vs full rides */}
      {/* <View style={styles.resultSummary}>
        <Text style={styles.resultSummaryText}>
          Found {processedRides.length} ride{processedRides.length !== 1 ? 's' : ''}
          {availableRideCount > 0 && ` • ${availableRideCount} available`}
          {fullRideCount > 0 && ` • ${fullRideCount} full`}
        </Text>
      </View> */}
