...
//   // Fetch route polyline for navigation
...
//             if (!lastPoint || 
//                 (Math.abs(lastPoint.latitude - latitude) > 0.00001 ||
//                  Math.abs(lastPoint.longitude - longitude) > 0.00001)) {
//               const newPath = [...prevPath, newLocation];
//               if (newPath.length > 1000) return newPath.slice(-1000);
//               return newPath;
//             }
//             return prevPath;
//           });
...
//           // Auto-fit map to show driver and pickup/dropoff
...
//       console.log('Error starting location tracking:', error);
//     }
//   }, [session?.session_id, socketConnected, pickupLocation, dropoffLocation]);
...
//         setPickupLocation({
//           latitude: res.data.pickup_lat,
//           longitude: res.data.pickup_lng
//         });
        
//         // Fetch route from driver's current location to pickup
//         if (driverLocation && res.data.pickup_lat && res.data.pickup_lng) {
//           fetchRoutePolyline(
...
//       if (res.data.dropoff_lat && res.data.dropoff_lng) {
//         setDropoffLocation({
//           latitude: res.data.dropoff_lat,
//           longitude: res.data.dropoff_lng
//         });
//       }
...
//       setRefreshing(false);
//     }
//   }, [rideId, user?.phone_number, driverLocation]);

//   // Notify riders that driver has reached pickup
//   const notifyRidersReachedPickup = async () => {
//     if (!session?.session_id) return;
    
//     try {
//       await axios.post(`${API_BASE_URL}/ride-sessions/${session.session_id}/driver-reached-pickup`, {
//         driver_phone: user?.phone_number,
//       });
      
//       showCustomAlert('Notified', 'Riders have been notified that you have reached the pickup location', 'success');
//     } catch (error) {
...
//       {/* Map View with Route Navigation */}
...
//         >
//           {/* Route to Pickup (during boarding phase) */}
//           {boardingPhase && routePolyline.length > 0 && (
//             <Polyline
...
//                 <Ionicons name="car-sport" size={32} color="#184080" />
...
//                 <Text style={styles.pickupMarkerLabel}>Pickup Point</Text>
//               </View>
//             </Marker>
//           )}

//           {/* Dropoff Location Marker */}
//           {dropoffLocation && enRoutePhase && (
...
//                 <Text style={styles.dropoffMarkerLabel}>Dropoff Point</Text>
//               </View>
//             </Marker>
//           )}
//         </MapView>
...
//         {/* Center Map Button */}
//         <TouchableOpacity style={styles.centerMapButton} onPress={centerMapOnDriver}>
//           <Ionicons name="locate" size={24} color="#184080" />
//         </TouchableOpacity>
        
//         {/* Navigate to Pickup Button */}
//         {boardingPhase && pickupLocation && (
//           <TouchableOpacity 
//             style={styles.navigateToPickupButton}
//             onPress={() => {
//               if (pickupLocation) {
//                 const url = Platform.select({
//                   ios: `maps://?q=${pickupLocation.latitude},${pickupLocation.longitude}`,
//                   android: `https://www.google.com/maps/search/?api=1&query=${pickupLocation.latitude},${pickupLocation.longitude}`,
//                 });
//                 Linking.openURL(url);
//               }
//             }}
...
//             <Text style={styles.navigateToPickupText}>Navigate to Pickup</Text>
//           </TouchableOpacity>
//         )}
        
//         {/* Notify Riders Button */}
//         {boardingPhase && driverLocation && pickupLocation && (
//           <TouchableOpacity 
//             style={styles.notifyRidersButton}
//             onPress={notifyRidersReachedPickup}
//           >
//             <Ionicons name="notifications" size={18} color="#fff" />
//             <Text style={styles.notifyRidersText}>Notify Riders: I've Arrived</Text>
//           </TouchableOpacity>
//         )}
//       </View>
...
//               <TouchableOpacity style={styles.mapCard} onPress={() => {
//                 if (dropoffLocation) {
//                   const url = Platform.select({
//                     ios: `maps://?q=${dropoffLocation.latitude},${dropoffLocation.longitude}`,
//                     android: `https://www.google.com/maps/search/?api=1&query=${dropoffLocation.latitude},${dropoffLocation.longitude}`,
//                   });
//                   Linking.openURL(url);
//                 }
//               }}>
...
//                           </Text>
//                           {boardedAt && (
//                             <Text style={[styles.timingText, { color: '#10B981' }]}>✓ Boarded at: {boardedAt}</Text>
//                           )}
//                           {droppedAt && (
//                             <Text style={[styles.timingText, { color: '#9333EA' }]}>✓ Dropped at: {droppedAt}</Text>
//                           )}
//                           {isRated && (
//                             <View style={styles.ratedBadge}>
//                               <Ionicons name="star" size={12} color="#F59E0B" />
...
//                 <Text style={styles.tripInfoLabel}>From</Text>
//                 <Text style={styles.tripInfoValue}>{session?.ride_origin?.split(',')[0] || 'Pickup'}</Text>
//                 {session?.ride_origin && pickupLocation && (
//                   <TouchableOpacity 
//                     style={styles.smallNavigateBtn} 
//                     onPress={() => {
//                       const url = Platform.select({
...
//                 <Text style={styles.tripInfoLabel}>To</Text>
//                 <Text style={styles.tripInfoValue}>{session?.ride_destination?.split(',')[0] || 'Destination'}</Text>
//                 {session?.ride_destination && dropoffLocation && (
//                   <TouchableOpacity 
//                     style={styles.smallNavigateBtn} 
//                     onPress={() => {
//                       const url = Platform.select({
...
const formatDateToIST = (dateTimeStr) => {
...
export default function OngoingRideDriverScreen({ route, navigation }) {
  ...
  const formatDateTime = (dateTimeStr) => {
    ...
    const date = new Date(dateTimeStr);
    return {
      date: formatDateToIST(dateTimeStr),
      time: formatToIST(dateTimeStr),
    };
  };
  ...
  const navigateToRiderPickup = (pickupLocation, riderName) => {
    if (!pickupLocation) {
      showCustomAlert('Location Not Available', `Pickup location for ${riderName} is not available`, 'warning');
      return;
    }
    ...
    if (typeof pickupLocation === 'object' && pickupLocation.latitude && pickupLocation.longitude) {
      lat = pickupLocation.latitude;
      lng = pickupLocation.longitude;
    } else if (typeof pickupLocation === 'string') {
      // If it's an address string, open maps with the address
      const encodedAddress = encodeURIComponent(pickupLocation);
      const url = Platform.select({
      ...
      showCustomAlert('Invalid Location', `Could not get coordinates for ${riderName}'s pickup`, 'warning');
      return;
    }
  ...
  // Navigate to rider's dropoff location
  const navigateToRiderDropoff = (dropoffLocation, riderName) => {
    if (!dropoffLocation) {
      showCustomAlert('Location Not Available', `Dropoff location for ${riderName} is not available`, 'warning');
      return;
    }
    
    let lat, lng;
    
    if (typeof dropoffLocation === 'object' && dropoffLocation.latitude && dropoffLocation.longitude) {
      lat = dropoffLocation.latitude;
      lng = dropoffLocation.longitude;
    } else if (typeof dropoffLocation === 'string') {
      const encodedAddress = encodeURIComponent(dropoffLocation);
      const url = Platform.select({
        ios: `maps://?q=${encodedAddress}`,
        android: `https://www.google.com/maps/search/?api=1&query=${encodedAddress}`,
      });
      ...
      showCustomAlert('Invalid Location', `Could not get coordinates for ${riderName}'s dropoff`, 'warning');
      return;
    }
          ...
          <View style={styles.card}>
            ...
            {!hasRiders ? (
              ...
              session?.riders?.map((rider, index) => {
                ...
                const boardedAt = rider.boarded_at ? formatToIST(rider.boarded_at) : null;
                const droppedAt = rider.dropped_off_at ? formatToIST(rider.dropped_off_at) : null;
                  ...
                  <View key={rider.id || rider.booking_id || index} style={styles.riderCard}>
                    <View style={styles.riderTop}>
                      <View style={styles.riderInfo}>
                        ...
                        <View style={{ flex: 1 }}>
                          ...
                          {boardedAt && (
                            <Text style={[styles.timingText, { color: '#10B981' }]}>✓ Boarded at: {boardedAt} IST</Text>
                          )}
                          {droppedAt && (
                            <Text style={[styles.timingText, { color: '#9333EA' }]}>✓ Dropped at: {droppedAt} IST</Text>
                          )}
                          {isRated && (
                            <View style={styles.ratedBadge}>
                              <Ionicons name="star" size={12} color="#F59E0B" />
                    ...
                    {!isCompleted && (
                      <View style={styles.riderActions}>
                        ...
                        {/* Navigate to Pickup Button for boarding phase */}
                        {boardingPhase && !isBoarded && rider.pickup_location && (
                          <TouchableOpacity 
                            style={[styles.navigateToRiderBtn, styles.flexButton]} 
                            onPress={() => navigateToRiderPickup(rider.pickup_location, rider.rider_name)}
                          >
                            <Ionicons name="navigate" size={16} color="#fff" />
                            <Text style={styles.navigateToRiderBtnText}>Navigate to Pickup</Text>
                          </TouchableOpacity>
                        )}

                        {/* Navigate to Dropoff Button for en route phase */}
                        {!boardingPhase && !isDropped && isBoarded && rider.dropoff_location && (
                          <TouchableOpacity 
                            style={[styles.navigateToRiderBtn, styles.flexButton, { backgroundColor: '#7C3AED' }]} 
                            onPress={() => navigateToRiderDropoff(rider.dropoff_location, rider.rider_name)}
                          >
                            <Ionicons name="navigate" size={16} color="#fff" />
                            <Text style={styles.navigateToRiderBtnText}>Navigate to Dropoff</Text>
                          </TouchableOpacity>
                        )}
