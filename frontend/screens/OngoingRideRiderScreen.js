...
export default function OngoingRideRiderScreen({ route, navigation }) {
  ...
  // Calculate ride progress percentage
  const calculateRideProgress = useCallback(() => {
    if (!driverLocation || !session?.dropoff_lat || !session?.dropoff_lng || rideCompleted) {
      return 0;
    }
  ...
  const checkSessionStatus = async () => {
    ...
    try {
      ...
      if (data.driver_reached_pickup) {
        setDriverReachedPickup(true);
        Alert.alert(
          'Driver Arrived!',
          'The driver has reached the pickup location. Please proceed to board the vehicle.',
          [{ text: 'OK' }]
        );
      }
  ...
  const syncSessionState = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        if (response.data.driver_reached_pickup && !driverReachedPickup) {
          setDriverReachedPickup(true);
          Alert.alert(
            'Driver Arrived!',
            'The driver has reached the pickup location. Please proceed to board the vehicle.',
  ...
  const connectSocket = useCallback(async (sessionId) => {
    ...
    try {
      ...
      socket.on('driver-reached-pickup', (data) => {
        console.log('🚗 Driver reached pickup event:', data);
        setDriverReachedPickup(true);
        Alert.alert(
          'Driver Arrived!',
          'The driver has reached the pickup location. Please proceed to board the vehicle.',
  ...
  const fetchSession = useCallback(async () => {
    ...
    try {
      ...
      if (res.data.driver_reached_pickup) {
        setDriverReachedPickup(true);
      }
  ...
  const handleBoardWithToken = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        // Check if it's the wrong rider's QR
        if (response.data.message && response.data.message.includes('not match')) {
          Alert.alert(
            'Invalid QR Code',
            'This QR code is for a different rider. Please ask the driver for YOUR personal boarding code.',
            [{ text: 'OK' }]
          );
        } else {
          Alert.alert('Boarding Failed', response.data.message || 'Could not board. Please try again.');
      ...
      if (errorMsg.includes('expired')) {
        Alert.alert('QR Code Expired', 'The QR code has expired. Please ask the driver to refresh the code.');
      } else if (errorMsg.includes('match') || errorMsg.includes('different rider')) {
        Alert.alert(
          'Wrong QR Code',
          'This QR code belongs to another rider. Please ask the driver to show you YOUR unique boarding code.',
          [{ text: 'OK' }]
        );
      } else {
        Alert.alert('Error', errorMsg);
  ...
  const showDriverETA = isWaitingForDriver && driverETA && !isReachedPickup && !driverReachedPickup;
  const showReachedPickupButton = isWaitingForDriver && !isReachedPickup && !rideCompleted && !driverReachedPickup;
  const showDriverArrivedMessage = driverReachedPickup && isWaitingForDriver && !isReachedPickup && !isBoarded;
  const showQRButton = (isReachedPickup || driverReachedPickup) && !isBoarded && !rideCompleted;
  const showDroppedOffButton = isBoarded && !isDroppedOff && !rideCompleted;
  const showCompleteButton = isDroppedOff && !rideCompleted;
      ...
      >
        {showRouteDetailsOnly ? (
          ...
          <>
            <View style={[
              styles.statusBanner, 
              { backgroundColor: rideCompleted ? '#F3F4F6' : isBoarded ? '#DCFCE7' : (isReachedPickup || driverReachedPickup) ? '#DCFCE7' : '#EFF6FF' }
            ]}>
              <Ionicons
                name={rideCompleted ? 'checkmark-done-circle' : isBoarded ? 'car-sport' : (isReachedPickup || driverReachedPickup) ? 'checkmark-circle' : 'time-outline'}
                size={24}
                color={rideCompleted ? '#6B7280' : isBoarded ? '#16A34A' : (isReachedPickup || driverReachedPickup) ? '#10B981' : '#F59E0B'}
              />
              <View style={styles.statusBannerText}>
                <Text style={[styles.statusBannerTitle, { color: rideCompleted ? '#374151' : isBoarded ? '#166534' : (isReachedPickup || driverReachedPickup) ? '#065A46' : '#92400E' }]}>
                  {rideCompleted ? 'Ride Completed' : isBoarded ? 'Onboard - Ride in Progress' : (isReachedPickup || driverReachedPickup) ? 'Ready to Board' : isWaitingForDriver ? 'Driver is on the way' : 'Waiting for driver'}
                </Text>
                <Text style={[styles.statusBannerSub, { color: rideCompleted ? '#6B7280' : isBoarded ? '#166534' : (isReachedPickup || driverReachedPickup) ? '#065A46' : '#92400E' }]}>
                  {rideCompleted 
                    ? 'Rate your experience with the driver' 
                    : isBoarded 
                    ? `Heading to ${session?.destination || 'destination'}` 
                    : (isReachedPickup || driverReachedPickup) 
                    ? 'Please enter the driver\'s boarding code to board'
                    : driverETA ? `${driverETA} mins away • ${formatDistance(driverDistance)}` : 'Waiting for driver to start...'}
                </Text>
              </View>
            ...
            {showDriverArrivedMessage && (
              <View style={styles.driverArrivedCard}>
                <Ionicons name="car-sport" size={24} color="#10B981" />
                <View style={styles.driverArrivedContent}>
                  <Text style={styles.driverArrivedTitle}>Driver has arrived!</Text>
                  <Text style={styles.driverArrivedText}>
                    The driver has reached the pickup location. Please proceed to board the vehicle.
                  </Text>
                </View>
              </View>
            ...
            <View style={styles.tripCard}>
              ...
              <View style={styles.tripRow}>
                <View style={styles.tripIconCol}>
                  <Ionicons name="ellipse" size={12} color={(isReachedPickup || driverReachedPickup) ? '#10B981' : '#9CA3AF'} />
                  <View style={[styles.dottedLine, (isReachedPickup || driverReachedPickup) && { backgroundColor: '#10B981' }]} />
                  <Ionicons name="flag" size={12} color={isDroppedOff ? '#10B981' : '#9CA3AF'} />
                </View>
                <View style={styles.tripTextCol}>
                  <Text style={styles.tripLabel}>Pickup</Text>
                  <Text style={[styles.tripValue, (isReachedPickup || driverReachedPickup) && { color: '#10B981' }]}>
                    {session?.origin || 'Pickup location'}
                    {(isReachedPickup || driverReachedPickup) && ' ✓'}
                  </Text>
                  <Text style={[styles.tripLabel, { marginTop: 16 }]}>Drop-off</Text>
            ...
            {/* I've Reached Pickup Button - Show before driver arrives */}
            {showReachedPickupButton && (
              <TouchableOpacity style={styles.reachedPickupBtn} onPress={handleReachedPickup}>
                <Ionicons name="location" size={20} color="#fff" />
                <Text style={styles.reachedPickupBtnText}>I've Reached Pickup Location</Text>
