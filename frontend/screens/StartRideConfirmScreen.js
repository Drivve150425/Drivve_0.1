...
export default function StartRideConfirmScreen({ route, navigation }) {
  ...
  const checkStartTime = (departureTimeStr) => {
    ...
    if (hoursSinceDeparture >= 2) {
      ...
      canStart = true;
      isLate = true;
      const remainingMinutes = Math.round(120 - minutesSinceDeparture);
      const lateMinutes = Math.round(minutesSinceDeparture);
      statusMessage = `⚠️ Ride is ${formatMinutesToHM(lateMinutes)} late. Start before auto-cancellation in ${formatMinutesToHM(remainingMinutes)}.`;
      statusColor = '#F59E0B';
    } else if (minutesToDeparture <= 15 && minutesToDeparture > 0) {
      canStart = true;
      const earlyMinutes = Math.round(minutesToDeparture);
      if (earlyMinutes <= 1) {
        statusMessage = `✅ Ready to start! You can start the ride now.`;
      } else {
        statusMessage = `✅ Ready to start! You can start the ride now (${formatMinutesToHM(earlyMinutes)} before departure).`;
      }
      statusColor = '#10B981';
    } else if (minutesToDeparture <= 0 && minutesToDeparture > -15) {
      canStart = true;
      ...
      canStart = false;
      const minutesUntilStart = Math.round(minutesToDeparture - 15);
      const timeText = formatTimeUntilStart(minutesUntilStart);
      statusMessage = `⏰ You can start the ride 15 minutes before departure (in ${timeText}).`;
      statusColor = '#6B7280';
    } else {
      canStart = true;
      statusMessage = `✅ Ready to start!`;
    ...
    setTimeStatus({
      canStart,
      minutesToDeparture: Math.round(Math.abs(minutesToDeparture)),
  ...
  const getDriverStatusInfo = () => {
    ...
    if (minutesSinceDeparture > 0 && minutesSinceDeparture < 120) {
      const remainingMinutes = Math.round(120 - minutesSinceDeparture);
      const lateMinutes = Math.round(minutesSinceDeparture);
      return { text: `Late (${formatMinutesToHM(lateMinutes)}) - ${formatMinutesToHM(remainingMinutes)} left`, color: "#EF4444", icon: "alert-circle" };
    }
    
    if (minutesToDeparture <= 15 && minutesToDeparture > 0) {
      const earlyMinutes = Math.round(minutesToDeparture);
      return { text: `Ready to start (${formatMinutesToHM(earlyMinutes)} early)`, color: "#10B981", icon: "checkmark-circle" };
    }
    
    if (minutesToDeparture <= 60 && minutesToDeparture > 15) {
      const waitMinutes = Math.round(minutesToDeparture);
      return { text: `Starts in ${formatMinutesToHM(waitMinutes)}`, color: "#F59E0B", icon: "time-outline" };
    }
    
    if (minutesToDeparture > 60) {
      const waitMinutes = Math.round(minutesToDeparture);
      return { text: `Starts in ${formatMinutesToHM(waitMinutes)}`, color: "#2457A6", icon: "calendar-outline" };
    }
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      >
        ...
        {timeStatus.isLate && (
          <View style={styles.lateWarningCard}>
            <Ionicons name="alert-circle" size={20} color="#DC2626" />
            <Text style={styles.lateWarningText}>
              Your ride is late! Please start as soon as possible to avoid auto-cancellation in {formatMinutesToHM(120 - timeStatus.minutesSinceDeparture)}.
            </Text>
          </View>
        )}
      ...
      <View style={[styles.footer, { paddingBottom: insets.bottom > 0 ? insets.bottom : 12 }]}>
        <View style={styles.warningBeforeButton}>
          <Ionicons name="information-circle" size={16} color="#F59E0B" />
          <Text style={styles.warningBeforeButtonText}>
            You can start 15 minutes before departure. You have 2 hours after departure to start the ride.
          </Text>
        </View>
        ...
        <TouchableOpacity
          style={[
            styles.startButton, 
            (!canStartNow || loading) && styles.startButtonDisabled,
            timeStatus.isLate && styles.startButtonLate
          ]}
          onPress={handleStartRide}
        ...
        >
          {loading ? (
            ...
            <>
              <Ionicons 
                name={timeStatus.isLate ? "alert-circle" : "car-sport-outline"} 
                size={22} 
                color="#fff" 
        ...
        {!canStartNow && !loading && timeStatus.minutesToDeparture > 15 && (
          <Text style={styles.disabledHint}>
            You can start the ride 15 minutes before departure ({formatTimeUntilStart(timeStatus.minutesToDeparture - 15)})
          </Text>
        )}
        ...
        {canStartNow && !loading && timeStatus.isLate && (
          <Text style={[styles.disabledHint, { color: '#DC2626' }]}>
            Hurry! Auto-cancellation in {formatMinutesToHM(120 - timeStatus.minutesSinceDeparture)}
          </Text>
        )}
      </View>
    </SafeAreaView>
