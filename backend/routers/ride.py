...
def reject_modification_request(request_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        booking.cancellation_reason = "Modification request rejected - Original booking cancelled"
        ...
        if ride.status == "full":
            remaining = ride.available_seats - total_booked_after
            if remaining > 0:
                ride.status = "active"
        ...
        return {
            "success": True,
            "message": f"Modification rejected. Booking for {original_seats} seat(s) cancelled.",
