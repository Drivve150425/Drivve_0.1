...
def update_ride(ride_id: int, data: UpdateRideRequest, db: Session = Depends(get_db)):
    ...
    # ============================================
    # NEW: Women Only toggle validation
    # ============================================
    ...
    if ride.women_only != new_women_only:
        # Trying to change Women Only setting
        if new_women_only == False and ride.women_only == True:
            # Trying to disable Women Only mode
            # Check if any female passengers have already booked
    ...
    # ============================================
    # NEW: Auto-reject pending requests if major changes
    # ============================================
    rejected_count = 0
...
def cancel_booking(booking_id: int, db: Session = Depends(get_db)):
    ...
    # Refund seats if accepted
    if booking.status == "accepted":
        # Note: We don't modify ride.available_seats directly
        # The available seats are calculated as ride.available_seats - total_booked
        # So when we cancel, total_booked decreases, available increases automatically
        if ride.status == "full":
            ride.status = "active"
...
def expire_pending_rides(db: Session = Depends(get_db)):
    """
    Auto-expire rides that haven't started within 45 minutes after scheduled time.
    Run this via cron job every 5 minutes.
    """
    now = datetime.now(timezone.utc)
    # Rides that were supposed to depart 45+ minutes ago but haven't started
...
def expire_pending_requests(db: Session = Depends(get_db)):
    """
    Auto-expire pending ride requests that are 15 minutes before ride start.
    Run this via cron job every 5 minutes.
    """
    now = datetime.now(timezone.utc)
    # Requests expire 15 minutes before ride departure
    ...
    pending_requests = db.query(RideBooking).join(Ride).filter(
        ...
        Ride.departure_time > now  # Only future rides, not past ones
    ).all()
...
@router.get("/ride/{ride_id}/expire-check")
def check_ride_expiry(ride_id: int, db: Session = Depends(get_db)):
    """Check if a ride should be expired and expire it if needed"""
    ride = db.query(Ride).filter(Ride.id == ride_id).first()
    if not ride:
        raise HTTPException(status_code=404, detail="Ride not found")
    ...
    if ride.departure_time < expiry_cutoff and ride.status in ["active", "full"]:
        # Expire the ride
        ride.status = "cancelled"
        ride.cancellation_reason = "Auto-expired: Ride did not start on time"
        ...
        return {
            "expired": True,
            "message": "Ride has been auto-expired",
            "affected_passengers": len(bookings)
        }
    
    return {
        "expired": False,
        "message": "Ride is still active",
        "time_until_expiry": max(0, (ride.departure_time + timedelta(minutes=45) - now).total_seconds() / 60)
    }


@router.get("/booking/{booking_id}/expire-check")
def check_booking_expiry(booking_id: int, db: Session = Depends(get_db)):
    """Check if a pending booking request should be expired"""
    booking = db.query(RideBooking).filter(RideBooking.id == booking_id).first()
    if not booking:
        raise HTTPException(status_code=404, detail="Booking not found")
    ...
    if ride.departure_time < expiry_threshold:
        # Expire the request
        booking.status = "rejected"
        ...
        return {
            "expired": True,
            "message": "Request has been auto-expired"
        }
    
    return {
        "expired": False,
        "message": "Request is still pending",
        "time_until_expiry": max(0, (ride.departure_time - timedelta(minutes=15) - now).total_seconds() / 60)
    }
...
#                 "passenger_photo": bk["passenger_profile_picture"],  # FIXED: Use correct field name
...
#     # Format posted rides with bookings and live session info
#     posted_formatted = []
#     for ride in posted_rides:
#         total_booked = get_total_booked_seats(db, ride.id)
#         remaining_seats = max(0, ride.available_seats - total_booked)
...
#             "live_session": live_session_data,
#             "driver_profile_picture": driver_profile_picture,  # Add driver's own profile picture
#         })
    
#     # REQUESTED RIDES (Passenger bookings) - FIXED to include driver profile picture
...
#             u.profile_picture as driver_profile_picture,  -- FIXED: Get driver's profile picture
...
#             "driver_photo": row["driver_profile_picture"],  # FIXED: Use driver's profile picture
...
class SeatModificationResponse(BaseModel):
    booking_id: int
    requested_seats: int
...
def request_seat_modification(
    booking_id: int, 
    data: SeatModificationRequest, 
    db: Session = Depends(get_db)
):
    """Step 1: Passenger requests to modify seat count, pending driver approval"""
    ...
    if data.requested_seats > available_seats_excluding_current + booking.seats_booked:
        raise HTTPException(
            status_code=400,
            detail=f"Only {available_seats_excluding_current + booking.seats_booked} total seats available. Cannot request {data.requested_seats} seats."
        )
    
    # Check if there's already a pending modification request
    existing_request = db.query(RideSeatModificationRequest).filter(
    ...
    if existing_request:
        raise HTTPException(
            status_code=400,
            detail="You already have a pending modification request. Please wait for driver approval."
        )
    ...
    db.add(modification_request)
    db.commit()
    db.refresh(modification_request)
    
    # Notify driver about the modification request
    try:
        origin_short = ride.origin.split(",")[0].strip() if ride.origin else "pickup"
        dest_short = ride.destination.split(",")[0].strip() if ride.destination else "destination"
        ...
        notification = UserNotification(
            phone_number=ride.phone_number,
            title="Seat Modification Request 🔄",
            message=f"Passenger wants to change from {booking.seats_booked} to {data.requested_seats} seat(s) for ride from {origin_short} to {dest_short}.",
            type=NotificationType.RIDE,
        ...
        db.add(notification)
        db.commit()
        print(f"✅ Modification request notification sent to driver {ride.phone_number}")
    except Exception as e:
        print(f"❌ Error sending modification notification: {str(e)}")
    ...
    try:
        ...
        print(f"Socket notification error: {str(e)}")
...
def approve_modification_request(request_id: int, db: Session = Depends(get_db)):
    """Step 2a: Driver approves the seat modification request"""
    ...
    # Check seat availability for the new requested seats
    total_booked = get_total_booked_seats(db, ride.id)
    other_booked = total_booked - booking.seats_booked
    ...
    if mod_request.requested_seats > available_seats_excluding_this:
        ...
        raise HTTPException(
            status_code=400,
            detail=f"Cannot approve: Only {available_seats_excluding_this} seat(s) available, but requested {mod_request.requested_seats}"
        )
    ...
    # Notify passenger about approval
    try:
        origin_short = ride.origin.split(",")[0].strip() if ride.origin else "pickup"
        dest_short = ride.destination.split(",")[0].strip() if ride.destination else "destination"
        ...
        notification = UserNotification(
            phone_number=booking.passenger_phone,
            title="Seat Modification Approved ✅",
            message=f"Your request to change from {old_seats} to {mod_request.requested_seats} seat(s) has been approved by the driver.",
            type=NotificationType.RIDE,
            action_type="booking",
            action_value=str(booking.id),
            is_read=False,
...
def reject_modification_request(
    request_id: int, 
    rejection_reason: Optional[str] = "Driver declined the modification request",
    db: Session = Depends(get_db)
):
    """Step 2b: Driver rejects the seat modification request"""
    ...
    try:
        notification = UserNotification(
            phone_number=booking.passenger_phone,
            title="Seat Modification Declined ❌",
            message=f"Your request to change seats was declined by the driver. Reason: {rejection_reason}",
            type=NotificationType.RIDE,
            action_type="booking",
            action_value=str(booking.id),
            is_read=False,
...
def get_pending_modification_request(booking_id: int, db: Session = Depends(get_db)):
    """Check if there's a pending modification request for a booking"""
...
def cancel_modification_request(booking_id: int, db: Session = Depends(get_db)):
    """Passenger cancels their pending modification request"""
    ...
    try:
        notification = UserNotification(
            phone_number=mod_request.driver_phone,
            title="Modification Request Cancelled",
            message=f"Passenger has cancelled their seat modification request.",
            type=NotificationType.RIDE,
            action_type="modification",
...
def get_pending_modifications_for_ride(ride_id: int, db: Session = Depends(get_db)):
    """Get all pending seat modification requests for a ride (for driver)"""
    ...
    for req in pending_requests:
        ...
        passenger_name = passenger.full_name or " ".join(
            filter(None, [passenger.first_name, passenger.last_name])
        ) if passenger else f"Passenger {req.passenger_phone[-4:]}"
...
async def get_my_rides(phone: str, db: Session = Depends(get_db)):
    ...
    for ride in posted_rides:
        ...
        # Get vehicle details - FIXED: Fetch vehicle data
        vehicle = None
        if ride.vehicle_id:
        ...
        posted_formatted.append({
            ...
            "status": display_status,
            "vehicle_id": ride.vehicle_id,
            "vehicle": vehicle_data,  # FIXED: Now includes complete vehicle data
            "route_coordinates": ride.route_coordinates,  # FIXED: Now includes route coordinates
            "suggested_pickup": ride.suggested_pickup,  # FIXED: Include if exists
            "suggested_drop": ride.suggested_drop,  # FIXED: Include if exists
            "created_at": ride.created_at.isoformat() if ride.created_at else None,
            "bookings": bookings_map.get(ride.id, []),
            "live_session": live_session_data,
