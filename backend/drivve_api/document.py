...
def upload_to_azure(file_bytes, file_path, content_type="image/jpeg"):
    ...
    Args:
        ...
        file_path: Path within the container (e.g., "documents/aadhar/front/123.jpg")
...
async def save_image(upload_file: UploadFile, image_type: str,
    ...
    # Upload to Azure
    return upload_to_azure(file_bytes, file_path, "image/jpeg")
    ...
    try:
        ...
        for doc in documents:
            ...
            result.append({
                ...
                "rejection_reason": doc.rejection_reason,
                "document_data": doc.document_data,
                "icon": get_document_icon(doc.document_type.value if hasattr(doc.document_type, 'value') else str(doc.document_type))
            })
    ...
    try:
        ...
        for doc in documents:
            ...
            result.append({
                ...
                "user_name": f"{user.first_name} {user.last_name}" if user else "Unknown",
                "user_email": user.email if user else None,
                "document_type": doc.document_type.value.upper() if hasattr(doc.document_type, 'value') else str(doc.document_type),
                "document_number": doc.document_number,
                "document_name": doc.document_name,
                "status": doc.status.value if hasattr(doc.status, 'value') else str(doc.status),
                "front_image_url": doc.front_image_path,
