...
# =========================
# CREATE SOCKET.IO ASGI APP - USE ONLY ONE
# =========================
# Create the Socket.IO ASGI app
socket_app = socketio.ASGIApp(sio, other_asgi_app=app)

# DO NOT mount socket_app again - use it as the main app
# The socket_app already wraps the FastAPI app
...
def setup_postgis_and_ride_columns():
    try:
        with engine.begin() as conn:
            ...
            conn.execute(text("ALTER TABLE ride_session_riders ADD COLUMN IF NOT EXISTS rider_rating INTEGER"))
            conn.execute(text("ALTER TABLE ride_session_riders ADD COLUMN IF NOT EXISTS rider_feedback TEXT"))
            conn.execute(text("ALTER TABLE ride_session_riders ADD COLUMN IF NOT EXISTS individual_qr_token VARCHAR(64) UNIQUE"))
            conn.execute(text("ALTER TABLE ride_session_riders ADD COLUMN IF NOT EXISTS qr_expires_at TIMESTAMP WITH TIME ZONE"))
            conn.execute(text("ALTER TABLE ride_session_riders ADD COLUMN IF NOT EXISTS seats_booked INTEGER DEFAULT 1"))
            conn.execute(text("ALTER TABLE ride_session_riders ADD COLUMN IF NOT EXISTS price_paid NUMERIC(10,2)"))
            print("✅ Ride session riders columns checked")
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        "main:socket_app",  # ← CHANGE THIS: Run socket_app instead of app
        host="0.0.0.0",
        port=8000,
        reload=True,
        access_log=True
